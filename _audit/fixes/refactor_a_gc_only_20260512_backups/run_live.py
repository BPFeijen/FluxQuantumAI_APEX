...
def _init_executor(broker: str) -> None:
    """Initialize the broker executor based on --broker argument OR BROKER env var.

    2026-05-11: when ENV BROKER=ctrader, skip MT5 RoboForex/Hantec init entirely.
    MT5 lib (MetaTrader5 Python package) attempts to connect to MT5 Terminal
    GUI; when terminal is closed (as it is now — broker switched to cTrader),
    the underlying Fortran runtime aborts the entire Python process via
    'forrtl: error (200): program aborting due to window-CLOSE event'. The
    cTrader executor is loaded by event_processor.py via the BROKER env var
    switch (line ~70), so MT5 path is unnecessary and dangerous.
    """
    global _executor, EXECUTOR_AVAILABLE
    _env_broker = os.environ.get("BROKER", "mt5").lower()
    if _env_broker == "ctrader":
        # cTrader path: event_processor handles broker init; no MT5 here.
        _executor = None
        EXECUTOR_AVAILABLE = False  # legacy flag; cTrader executor is owned by event_processor
        print(_color("[BROKER] BROKER=ctrader env var detected — MT5 init skipped "
                     "(event_processor will load CTraderExecutor).", _GREEN))
        return
    if broker == "hantec":
        try:
...
def _run_event_driven(args) -> None:
    ...
    # 2026-05-11: defer PositionMonitor init until AFTER processor is created
    # (we need processor.executor when BROKER=ctrader because the legacy
    # _executor is None). The actual instantiation happens further below,
    # after `processor = _EventProcessor(...)`. The variable `monitor` is
    # kept None here so the optional IPC injection at line ~866 can detect
    # the still-uninitialized state.
    monitor = None
    if _executor is not None:
        ...
        print(_color("[INFO] PositionMonitor init deferred until after EventProcessor (BROKER=ctrader path)", _YELLOW))
