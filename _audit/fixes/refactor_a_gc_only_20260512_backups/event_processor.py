...
#
# 2026-05-11 (Barbara directive): skip Hantec init when BROKER=ctrader.
# The MetaTrader5 Python lib (imported by mt5_executor_hantec) uses a
# Fortran-backed runtime that aborts the entire Python process via
# 'forrtl: error (200): program aborting due to window-CLOSE event' when
# the MT5 Terminal GUI it tries to attach to is not running. Since the
# broker is now cTrader and MT5 terminals are not running, instantiating
# the Hantec executor crashes the trading service in a restart loop.
...
#
# 2026-05-11 (Barbara directive): force-disabled. The underlying provider
# (TradingEconomics guest:guest tier) returns HTTP 410 Gone permanent and
# SSL EOF errors. Each tick_loop iteration blocked on retry+timeout on the
# main thread, freezing decision emission post-market-open. The hybrid
# Finnhub+Alpha Vantage migration was applied only to the dashboard
# /api/news route, not to the trading service news gate.
#
# Until the trading service's news gate is migrated off TradingEconomics,
# we disable it so signal emission can proceed. News-based entry blocks
# are unavailable while this is active; operator must be aware of major
# scheduled events (CPI, FOMC, NFP) via the dashboard calendar.
_news_gate = None  # type: ignore[assignment]
logging.getLogger("apex.event").warning(
    "ApexNewsGate FORCE-DISABLED 2026-05-11 — TradingEconomics provider dead; "
    "trading without news gate. See dashboard /api/news for Finnhub calendar."
)
...
class EventProcessor:
    ...
    def __init__(
        ...
        # 2026-05-11: Force broker connect at startup so we don't wait until
        # first GO signal to attempt reconnect (which leaves a window of GO
        # signals firing into EXEC_FAILED while connection establishes).
        # For cTrader executors the constructor only configures account; the
        # actual TCP+TLS+auth handshake happens in reconnect(). Call it now,
        # log result. Failure here is non-fatal (open_position path will
        # retry); but if it succeeds, heartbeat thread starts now and the
        # connection is alive for the first signal that fires.
        try:
            _ok = self.executor.reconnect()
            log.info("Startup broker reconnect: %s", "OK" if _ok else "FAILED (will retry on first signal)")
            print(f"[BROKER] Startup reconnect: {'OK' if _ok else 'FAILED — will retry on first signal'}")
        except Exception as _conn_e:
            log.warning("Startup broker reconnect raised: %s", _conn_e)
            print(f"[BROKER] Startup reconnect ERROR: {_conn_e}")
    ...
    def _open_on_all_accounts(
        ...
        # Deadlock fix: try reconnect BEFORE checking connected flag.
        # Executors use deferred init; .connected starts False and only flips
        # True after reconnect() runs, but reconnect() was previously only
        # invoked inside open_position() — which was gated by .connected.
        if not self.executor.connected:
            try:
                self.executor.reconnect()
            except Exception as _rx:
                log.warning("RoboForex reconnect pre-check failed: %s", _rx)
        ...
        # Deadlock fix: try reconnect BEFORE checking connected flag (Hantec path).
        if self.executor_live is not None and not self.executor_live.connected:
            try:
                self.executor_live.reconnect()
            except Exception as _rx:
                log.warning("Hantec reconnect pre-check failed: %s", _rx)
    ...
    def _metric_incr(self, name: str, value: int = 1) -> None:
        """Lightweight structured-log metric helper (Sprint A entry_logic_fix_20260420).

        Emits ``METRIC <name> <value>`` on the apex logger so log-scrapers can
        aggregate without needing a real metrics backend yet.
        """
        try:
            log.info("METRIC %s %d", name, value)
        except Exception:
            pass

    def _classify_source_from_candidate(self, cand) -> str:
        """Map LevelCandidate.source to legacy ``_near_level_source`` vocabulary."""
        try:
            src = getattr(cand, "source", "")
            if src.startswith("m5"):
                return "m5_only"
            if src.startswith("m30"):
                return "m30_only"
        except Exception:
            pass
        return ""
    ...
    def _near_level(
        ...
        Two modes:
          * ``direction=None`` (legacy): returns ``(level_type, level_price)`` — byte-compatible
            with pre-Sprint-A behaviour (see ``_near_level_legacy``). Used where direction is not
            yet known (e.g. initial discovery in tick loop + iceberg path).
          * ``direction in ("SHORT","LONG")``: returns ``(status, LevelCandidate | None)`` where
            status is ``"PASS" | "NEAR" | "FAR"`` with direction-aware filtering per
            literatura-aligned ordering (Wyckoff/ICT/ATS).

        Used by Sprint A C1 post-validation: after legacy discovery + ``_resolve_direction``,
        callers invoke this with the resolved direction to suppress wrong-side-level fires
        (03:14 bug class).
        """
        ...
        if direction not in ("SHORT", "LONG"):
            log.warning("_near_level: invalid direction=%r; deferring to legacy", direction)
            return self._near_level_legacy(price)

        try:
            from live.level_detector import get_levels_for_direction
        except Exception as e:
            log.warning("_near_level: level_detector import failed (%s); deferring to legacy", e)
            return self._near_level_legacy(price)

        atr = self._metrics.get("atr_m30_parquet", self._metrics.get("atr", 20.0))
        band = max(atr * NEAR_ATR_FACTOR, NEAR_FLOOR_PTS)
        offset = getattr(self, "_gc_xauusd_offset", None)
        ...
        try:
            candidates = get_levels_for_direction(
                ...
                gc_to_mt5_offset=offset,
            )
        except Exception as e:
            log.warning("_near_level: get_levels_for_direction failed (%s); deferring to legacy", e)
            return self._near_level_legacy(price)
        ...
        if not valid:
            ...
            log.info(
                "NEAR_LEVEL FAR: direction=%s price=%.2f -- %d candidates, all wrong side or none",
        ...
        top = valid[0]  # pre-sorted literatura-aligned
        self._near_level_source = self._classify_source_from_candidate(top)

        if top.distance_to_price <= top.band:
            log.info(
                "NEAR_LEVEL PASS: direction=%s price=%.2f level=%.2f dist=%.1f band=%.1f "
                "source=%s age=%.1fmin is_touch=%s",
                direction, price, top.level, top.distance_to_price, top.band,
                top.source, top.age_min, top.is_touch,
            )
            self._metric_incr("near_level.pass." + top.source)
            return "PASS", top

        log.info(
            "NEAR_LEVEL NEAR: direction=%s price=%.2f level=%.2f dist=%.1f band=%.1f "
            "source=%s age=%.1fmin (waiting approach)",
            direction, price, top.level, top.distance_to_price, top.band,
            top.source, top.age_min,
        )
    ...
    def _trigger_gate(self, price: float, direction: str, source: str,
        ...
        #   trending branch: adds COUNTER_HTF label when direction opposes daily_trend.
        #     COUNTER_HTF is LOG ONLY -- does NOT block (gate.check already decided GO/BLOCK).
        #   non-trending branch: uses direction-aware _near_level so v1 reflects FAR/NEAR/PASS
        #     using Wyckoff/ICT/ATS ordering instead of simple abs-distance.
        ...
        if _trending_v1:
            ...
            if _in_zone and _htf_aligned:
                ...
                self._metric_incr("v1.counter_htf." + direction.lower())
            else:
                v1 = "ZONE_FAIL"
        else:
            _ne_status, _ne_cand = self._near_level(price, direction=direction)
            v1 = _ne_status  # "PASS" | "NEAR" | "FAR"
        ...
        # Pre-compute SL / TP / lots BEFORE decision dict build so that
        # decision_log entry AND telegram GO notification carry the levels.
        # This mirrors the authoritative compute below but runs early to feed
        # the snapshot. Safe because inputs are direction + price + self state.
        _sl_pre, _tp1_pre, _tp2_pre = 0.0, 0.0, 0.0
        _lots_pre: list = []
        if decision.go:
            _is_cont_pre = "CONTINUATION" in strategy_reason
            if _is_cont_pre:
                _atr_pre = self._metrics.get("atr_m30_parquet", self._metrics.get("atr", 20.0))
                _tp1_mult_pre = float(self._thresholds.get("trend_cont_tp1_atr_mult", 0.8))
                _tp2_mult_pre = float(self._thresholds.get("trend_cont_tp2_atr_mult", 1.5))
                _stop_mode_pre = self._thresholds.get("trend_cont_stop_mode", "displacement_bar")
                _tp1_pts_pre = _atr_pre * _tp1_mult_pre
                _tp2_pts_pre = _atr_pre * _tp2_mult_pre
                if _stop_mode_pre == "displacement_bar":
                    try:
            ...
            try:
                _dyn_pre = self._compute_session_lots(
                    ice_aligned=decision.iceberg.aligned if decision.iceberg.detected else False,
                )
                _lots_pre = list(_dyn_pre) if _dyn_pre else list(_split_lots(self.lot_size))
            except Exception:
                _lots_pre = list(_split_lots(self.lot_size))
        ...
        _decision_dict = self._build_decision_dict(
            ...
            sl=_sl_pre, tp1=_tp1_pre, tp2=_tp2_pre, lots=_lots_pre,
        )
    ...
    def _on_iceberg_event(self, event: dict):
        ...
        # After legacy discovery + _resolve_direction, verify at least one literatura-valid
        # level exists on the correct side for the resolved direction. Suppresses 03:14-class
        # wrong-side-level fires (SHORT with liq_top BELOW price) without touching strategy logic.
        _pv_status, _pv_cand = self._near_level(xau_price, direction=direction)
        if _pv_status != "PASS":
            log.info(
                "POST_VALIDATION %s suppress [ICEBERG]: dir=%s level_type=%s level=%.2f price=%.2f",
                _pv_status, direction, level_type, level_price, xau_price,
            )
            self._metric_incr("near_level.post_validation.iceberg." + _pv_status.lower())
            print(f"[{_ts()}] [POST_VAL {_pv_status}] ICEBERG SUPPRESSED: dir={direction} "
                  f"level_type={level_type} level={level_price:.2f} (wrong-side/stale)")
            return
        ...
        while self._running:
            ...
            if gc_price <= 0:
                ...
                if level_type:
                    ...
                    if _pv_status != "PASS":
                        log.info(
                            "POST_VALIDATION %s suppress [TICK]: dir=%s level_type=%s level=%.2f price=%.2f",
                            _pv_status, direction, level_type, level_price, xau_price,
                        )
