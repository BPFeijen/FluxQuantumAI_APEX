...
# DEFERRED init: do NOT call mt5.initialize() at import time. Mirrors
# mt5_executor.py (RoboForex) pattern. The MetaTrader5 module is a process-global
# singleton — module-level init here would bind the singleton to Hantec and
# silently prevent the RoboForex executor from ever reconnecting.
# Connection happens lazily via MT5ExecutorHantec.reconnect().
# ---------------------------------------------------------------------------
MT5_AVAILABLE = False
mt5 = None
...
try:
    ...
    mt5 = _mt5
    log.info("Hantec MT5 module loaded — connection deferred to first reconnect() call")
except ImportError:
    _mt5 = None
    log.warning("MetaTrader5 not installed — Hantec executor running in dry-run mode")
